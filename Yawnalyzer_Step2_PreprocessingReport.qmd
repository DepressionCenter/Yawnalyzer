...
def add_alternating_backgrouds(df, time_col, day_start=6, night_start=18, night_color="rgba(87, 132, 210, .4)", day_color="rgba(250, 235, 215, .4)"):
    df_copy = df.copy()
    df_copy[time_col] = to_standardized_timezone(df_copy[time_col],time_zone="America/Detroit")
    df_copy[time_col] = df_copy[time_col]
    time_series = df_copy[time_col]
...
accel_df = pd.read_csv(os.path.join(PathKeeper.merged_data_path,"accel_collapsed.csv"), index_col=["index_1"])
...
accel_df = gait_df[accel_df["ID"] == RID].copy()
...
accel_df["TimestampISO"] = (
    accel_df["TimestampISO"]
    .infer_objects()
    .fillna(pd.to_datetime(accel["Time"], unit="s", utc=True))
)
...
accel_df["TimestampISO] = to_standarized_timezone(accel_df["TimestampISO"], "America/Detroit")
...
if has_hr:
    ...
    hr_fig.update_layout(
        shapes=add_alternating_backgrouds(hr_sorted_df,"TimestampISO", day_start=6, night_start=18),
...
if has_sleep:
    ...
    time_fig.update_layout(
        ...
        shapes=add_alternating_backgrouds(time_sorted_df,"GlobalTime", day_start=6, night_start=18),
        template="plotly_white"
    )
...
if has_accel:
    display(prettify_info(accel_df))
else:
...
if has_gait:
    ...
    gait_fig.update_layout(
        shapes=add_alternating_backgrouds(gait_sorted_df,"TimestampISO", day_start=6, night_start=18),
        template="plotly_white"
    )
...
if has_double_support:
    ...
    double_support_fig.update_layout(
        shapes=add_alternating_backgrouds(double_support_sorted_df,"TimestampISO", day_start=6, night_start=18),
        template="plotly_white"
    )
...
#| message: false
if has_sleep_survey:
    sleep_survey_df["Sleep03"] = sleep_survey_df["Sleep03"].fillna(1).astype("str")
    sleep_survey_df["Sleep03"] = sleep_survey_df["Sleep03"].replace({"1":"New Symptoms", "0":"No New Symptoms"})
...
csv_name = "All_Data_by_30sBins_"+RID+".csv"

all_data_df.to_csv(os.path.join(PathKeeper.summarized_data_path,csv_name), index=False)
...
sleep_range_df["Night_of_Sleep"] = pd.to_datetime(sleep_range_df["Night_of_Sleep"], errors="coerce", unit="ns").dt.tz_localize("America/Detroit")
...
if sleep_range_df.empty or len(sleep_range_df["Night_of_Sleep"]) < 1:
    ...
    sleep_df2["Night_of_Sleep"] = pd.to_datetime(sleep_df["Night_of_Sleep"], errors="coerce", unit="ns").dt.tz_localize("America/Detroit")
...
survey_fig.update_layout(showlegend=False, template="plotly_white")
survey_fig
...
total_fig.update_layout(showlegend=False,template="plotly_white")
total_fig
...
fig.update_layout(title="Shared Plot", template="plotly_white")
fig
```
...
#| eval: false
#| 
sleep_stage = all_data_df["Asleep"].replace([0,1],["W","S"])

hyp = yasa.Hypnogram(sleep_stage, n_stages=2, freq='30s')

hyp2 =yasa.plot_hypnogram(hyp)
