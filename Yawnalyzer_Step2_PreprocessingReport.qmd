...
gait_df["TimestampISO"] = (
    gait_df["TimestampISO"]
    .infer_objects()
    .fillna(pd.to_datetime(gait_df["Timestamp"], unit="s", utc=True).dt.tz_convert('America/New_York'))
)
# Double Support
...
double_support_df["TimestampISO"] = (
    double_support_df["TimestampISO"]
    .infer_objects()
    .fillna(pd.to_datetime(double_support_df["Timestamp"], unit="s", utc=True).dt.tz_convert('America/New_York'))
)
# Asymmetry
...
asymmetry_df["TimestampISO"] = (
    asymmetry_df["TimestampISO"]
    .infer_objects()
    .fillna(pd.to_datetime(asymmetry_df["Timestamp"], unit="s", utc=True).dt.tz_convert('America/New_York'))
)
# Cognitive survey
...
if has_gait:
    ...
    gait_sorteddf = gait_df.sort_values(by="TimestampISO")

    fig = go.Figure()

    fig.add_trace(
    go.Scatter(
        x=gait_sorteddf["TimestampISO"],
        y=gait_sorteddf["Balance"],
        mode="lines+markers",
        marker=dict(
        color="red"
        ),
    ...
    fig.add_hline(y=0.5, line_dash="dash", line_color="green", annotation_text="Balanced Gait")
    fig.show()
else:
    display(Markdown(f"_**No Gait Data Exists**_"))
```
...
```{python}
#| eval: true
all_df = pd.merge(hr_sorted_df[["ID","file_type", "TimestampISO"]], sleep_df[["ID", "file_type", "StartISO","Duration_in_Minutes"]], left_on="TimestampISO", right_on="StartISO", how="outer")

# all_df = all_df.merge(sleep_df)
...
new2 = pd.merge_asof(new_order,gait_df[['TimestampISO', "Balance"]], left_on="GlobalTime", right_on="TimestampISO",direction="nearest",
    tolerance=pd.Timedelta(seconds=15) )
```
