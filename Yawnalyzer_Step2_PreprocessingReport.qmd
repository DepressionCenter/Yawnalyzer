...
```{python}
 if has_sleep:
    q = sleep_df.sort_values(by="Night_of_Sleep")
    qq = (q.groupby(["ID", "Night_of_Sleep"], as_index=False).agg(
        Total_Durations =("Duration","sum")
    ).reset_index()
    
    )

    qq["Total_Durations"] = (qq["Total_Durations"]/3_600_000_000_000)

    fig = go.Figure()

    for day in qq["Night_of_Sleep"].unique():
        filtered_df = qq[qq["Night_of_Sleep"] == day]
        
        fig.add_trace(
            go.Bar(x=filtered_df["Night_of_Sleep"], 
            y=filtered_df["Total_Durations"], name=str(day)
            )
        )
    ...
    fig.add_hline(y=12, line_dash="dash", line_color="green", annotation_text="12 Hours of Sleep")
    fig.show()
```
...
if has_gait:
    q = gait_df.sort_values(by="TimestampISO")
...
HR_summary_df.to_csv(os.path.join(PathKeeper.summarized_data_path,csv_name))
```
...
sleep_all_df = pd.merge(sleep_days_df, sleep_df, left_on="GlobalDay", right_on="Night_of_Sleep", how="left").sort_values(["GlobalDay", "StartISO"])
...
sleep_summary_df.to_csv(os.path.join(PathKeeper.summarized_data_path,csv_name))
