...
def add_alternating_backgrouds(df, time_col, day_start=6, night_start=18, night_color="rgba(87, 132, 210, .4)", day_color="rgba(250, 235, 215, .4)"):
    df_copy = df.copy()
    df_copy[time_col] = to_standarized_timezone(df_copy[time_col],time_zone="America/Detroit")
...
if has_double_support:
    ...
    double_support_sorted_df = double_support_df.sort_values(by="TimestampISO")
    double_support_sorted_df["TimestampISO"] = pd.to_datetime(double_support_sorted_df["TimestampISO"], errors="coerce").dt.tz_convert("America/Detroit")
    display(Markdown(f"_**No Double Support Data Exists**_"))
```
