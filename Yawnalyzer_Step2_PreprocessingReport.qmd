...
if has_gait:
    ...
    gait_sorted_df = gait_df.sort_values(by="TimestampISO")
    gait_sorted_df["TimestampISO"] = pd.to_datetime(gait_sorted_df["TimestampISO"], errors="coerce").dt.tz_localize("America/Detroit")
    display(Markdown(f"_**No Gait Data Exists**_"))
```
...
sleep_range_df = pd.to_datetime(sleep_range_df["Night_of_Sleep"], errors="coerce").dt.tz_localize("America/Detroit")

if sleep_range_df.empty:
    ...
    overall_summary["Days_Missing_Sleep"] = 0
...
add_fig_traces(physio_fig, globals().get("sleep_fig"), start_row=4, end_row=4, yaxis_range=[0, None])
...
add_fig_traces(total_fig, globals().get("sleep_fig"), start_row=4, end_row=4, yaxis_range=[0, None])
...
all_traces += add_traces_single_plot(sleep_fig) if "sleep_fig" in globals() else []
